...
import { makeProviders, makeStandardFetcher, targets, NotFoundError } from '@movie-web/providers';
...
storage.mount('data', unstorage_47drivers_47fs_45lite({"driver":"fsLite","base":"L:\\addons\\stremify-2.7\\.data\\kv"}));
...
function d(e, r, s = ".", n) {
  ...
  for (const t in e) {
    if (t === "__proto__" || t === "constructor")
      continue;
    const i = e[t];
    i != null && (n && n(o, t, i, s) || (Array.isArray(i) && Array.isArray(o[t]) ? o[t] = [...i, ...o[t]] : u(i) && u(o[t]) ? o[t] = d(
      i,
      o[t],
      (s ? `${s}.` : "") + t.toString(),
      n
...
function S(e) {
  return (...r) => (
    // eslint-disable-next-line unicorn/no-array-reduce
    r.reduce((s, n) => d(s, n, "", e), {})
  );
}
const j = S();
class x extends Error {
  constructor(r, s = {}) {
    super(r, s), this.statusCode = 500, this.fatal = !1, this.unhandled = !1, s.cause && !this.cause && (this.cause = s.cause);
  }
  toJSON() {
    const r = {
      message: this.message,
    ...
    return this.statusMessage && (r.statusMessage = b(this.statusMessage)), this.data !== void 0 && (r.data = this.data), r;
  }
}
x.__h3_error__ = !0;
function h(e) {
  ...
  for (const s in e.node.req.headers) {
    const n = e.node.req.headers[s];
    r[s] = Array.isArray(n) ? n.filter(Boolean).join(", ") : n;
  }
  return r;
}
function c(e, r) {
  return h(e)[r.toLowerCase()];
}
const E = /[^\u0009\u0020-\u007E]/g;
function b(e = "") {
  return e.replace(E, "");
...
function H(e, r = 200) {
  return !e || (typeof e == "string" && (e = Number.parseInt(e, 10)), e < 100 || e > 999) ? r : e;
}
function y(e, r) {
  if (e.handled)
    return;
  const s = H(r, 204);
  s === 204 && e.node.res.removeHeader("content-length"), e.node.res.writeHead(s), e.node.res.end();
}
function M(e, r) {
...
const a = M;
function B(e, r, s) {
  let n = e.node.res.getHeader(r);
  if (!n) {
    e.node.res.setHeader(r, s);
    return;
  }
  Array.isArray(n) || (n = [n.toString()]), e.node.res.setHeader(r, [...n, s]);
}
function N(e = {}) {
  return j(e, {
    origin: "*",
...
function p(e) {
  const r = c(e, "origin"), s = c(
    e,
    "access-control-request-method"
  );
  return e.method === "OPTIONS" && !!r && !!s;
}
function f(e, r) {
  const { origin: s } = r;
  return !e || !s || s === "*" || s === "null" ? !0 : Array.isArray(s) ? s.some((n) => n instanceof RegExp ? n.test(e) : e === n) : s(e);
}
function O(e, r) {
  const { origin: s } = r, n = c(e, "origin");
  return !n || !s || s === "*" ? { "access-control-allow-origin": "*" } : typeof s == "string" ? { "access-control-allow-origin": s, vary: "origin" } : f(n, r) ? { "access-control-allow-origin": n, vary: "origin" } : {};
}
function P(e) {
  const { methods: r } = e;
  return r ? r === "*" ? { "access-control-allow-methods": "*" } : r.length > 0 ? { "access-control-allow-methods": r.join(",") } : {} : {};
}
function R(e) {
  const { credentials: r } = e;
  return r ? { "access-control-allow-credentials": "true" } : {};
}
function T(e, r) {
  const { allowHeaders: s } = r;
  if (!s || s === "*" || s.length === 0) {
    const n = c(e, "access-control-request-headers");
    return n ? {
...
function A(e) {
  const { exposeHeaders: r } = e;
  return r ? r === "*" ? { "access-control-expose-headers": r } : { "access-control-expose-headers": r.join(",") } : {};
}
function w(e, r) {
  a(e, O(e, r)), a(e, R(r)), a(e, A(r)), a(e, P(r)), a(e, T(e, r));
}
function m(e, r) {
  a(e, O(e, r)), a(e, R(r)), a(e, A(r));
}
function z(e, r) {
  const s = N(r);
  return p(e) ? (w(e, r), y(e, s.preflight.statusCode), !0) : (m(e, r), !1);
}
function q(e) {
  if (typeof e == "function")
    return Object.assign(e, { __is_handler__: !0 });
  const r = {
    onRequest: l(e.onRequest),
    onBeforeResponse: l(e.onBeforeResponse)
  };
  return Object.assign((n) => D(n, e.handler, r), { __is_handler__: !0 });
}
function l(e) {
  return e ? Array.isArray(e) ? e : [e] : void 0;
}
async function D(e, r, s) {
  if (s.onRequest) {
    for (const t of s.onRequest)
      if (await t(e), e.handled)
        return;
  }
  const o = { body: await r(e) };
...
const J = (e, r) => q((s) => (z(s, r), e(s)));

const _UGPm8R = J((_event) => {
}, {
  origin: "*",
  methods: "*"
});

async function voeResolve(link) {
  try {
...
async function uqloadResolve(link) {
  try {
    const response = await fetch(link, { redirect: "follow" });
    if (response.ok) {
      const embed = await response.text();
...
const baseurl$6 = "https://frembed.fun/";
async function scrapeFrembed(imdbid, season, episode) {
  let url = "";
  if (episode == 0 || episode == "0") {
    url = `${baseurl$6}/api/film.php?id=${imdbid}`;
  } else {
    url = `${baseurl$6}/api/serie.php?id=${imdbid}&sa=${season}&epi=${episode}`;
  }
  const finalstreams = [];
  try {
    ...
    const streamsPromises = matches.map(async (match) => {
      ...
      if (decodedurl.includes("voe.sx")) {
        const url2 = await voeResolve(new URL(decodedurl));
        return {
          name: "Stremify FR",
          type: "url",
          url: url2,
        ...
        const url2 = await uqloadResolve(new URL(decodedurl));
        return {
          name: "Stremify FR",
          type: "url",
          url: url2,
    ...
    return finalstreams.filter((stream) => stream !== void 0);
  } catch (error) {
    return null;
  }
}

async function superivdeodroploadResolve(link) {
  ...
  if (script != null) {
    ...
    if (stream == null ? void 0 : stream.groups) {
...
const baseurl$5 = "https://kinokiste.live";
async function scrapeKinokiste(imdb, season, episode) {
  const finalstreams = [];
  try {
    const response = await fetch(`${baseurl$5}/serien/?do=search&subaction=search&story=${imdb}"`, {
      headers: {
    ...
    if (droploadmatch != null) {
      const url = await superivdeodroploadResolve(new URL(droploadmatch[1].replace(/(.io\/)(.*)/, "$1e/$2")));
      finalstreams.push(
        {
          name: "Stremify DE",
          type: "url",
    ...
    if (supervideomatch != null) {
      const url = await superivdeodroploadResolve(new URL(supervideomatch[1].replace(/(.tv\/)(.*)/, "$1e/$2").replace(".html", "")));
      finalstreams.push(
        {
          name: "Stremify DE",
          type: "url",
...
async function streamtapeResolve(link) {
  try {
    const response = await fetch(link, { redirect: "follow" });
    if (response.ok) {
...
const baseurl$4 = "https://meinecloud.click/";
async function scrapeMeinecloud(imdbid) {
  const finalstreams = [];
  const url = `${baseurl$4}/movie/${imdbid}`;
  try {
    const response = await fetch(url);
    if (!response.ok) {
      return null;
    ...
    while ((match = droploadregex.exec(text)) !== null) {
      ...
      const url2 = await superivdeodroploadResolve(new URL(embedurl));
      finalstreams.push({
        name: "Stremify DE",
        type: "url",
        url: url2,
...
const baseurl$3 = "https://eurostreaming.forum/";
async function scrapeEurostreaming(imdb, season, episode) {
  const finalstreams = [];
  try {
    const response = await fetch(`${baseurl$3}/index.php`, {
      method: "POST",
      headers: {
        "User-Agent": "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36",
...
const baseurl$2 = "https://guardahd.stream";
async function scrapeGuardahd(imdbid) {
  const finalstreams = [];
  const url = `${baseurl$2}/movie/${imdbid}`;
  try {
    const response = await fetch(url, {
      headers: {
        "User-Agent": "Mozilla/5.0 (Windows NT 6.1) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2228.0 Safari/537.36"
...
const baseurl$1 = "https://verhdlink.cam";
async function scrapeVerdahd(imdbid) {
  const finalstreams = [];
  const url = `${baseurl$1}/movie/${imdbid}`;
  try {
    const response = await fetch(url);
    if (!response.ok) {
      return null;
...
async function scrapeCinehdplus(imdb, season, episode) {
  const finalstreams = [];
  try {
    ...
    const searchResponse = await response.text();
    const searchRegex = /<div class="card__cover"><a href="[^"]*">[\s\S]*? class="idioma" alt="[^"]*">/g;
    const searchMatch = searchRegex.exec(searchResponse);
    const urlregex = /<a href="([^"]*)">/;
    const showUrl = urlregex.exec(searchMatch[0]);
    let lang = "";
    if (searchMatch[0].includes("latino")) {
      lang = "Latino";
    } else {
      lang = "Castellano";
    }
    ...
    const episodeData = await episodeFetch.text();
    const droploadregex = new RegExp(`data-num="${season}x${episode}".*?data-link="(https://dropload\\.io/embed-\\w+\\.html)"`, "s");
    const droploadmatch = droploadregex.exec(episodeData);
    if (droploadmatch != null) {
...
async function makeTMDBRequest(url, appendToResponse) {
  const headers = {
...
async function convertImdbIdToTmdbId(imdbId) {
...
const providers = makeProviders({
  ...
  // check out https://movie-web.github.io/providers/essentials/targets
});
async function getMedia(media, source) {
  let output;
  try {
  ...
  if (embeds) {
    const scrapePromises = embeds.map(
      (embed) => providers.runEmbedScraper({
        id: embed.embedId,
        url: embed.url
...
const movies = /* @__PURE__ */ new Map([
  ["frembed", async (imdbid) => await scrapeFrembed(imdbid, 0, 0)],
  ["meinecloud", async (imdbid) => await scrapeMeinecloud(imdbid)],
  ["guardahd", async (imdbid) => await scrapeGuardahd(imdbid)],
  ["verhdlink", async (imdbid) => await scrapeVerdahd(imdbid)]
]);
const series = /* @__PURE__ */ new Map([
  ["kinokiste", async (imdbid, season, episode) => await scrapeKinokiste(imdbid, season, episode)],
  ["cinehdplus", async (imdbid, season, episode) => await scrapeCinehdplus(imdbid, season, episode)],
  ["frembed", async (imdbid, season, episode) => await scrapeFrembed(imdbid, season, episode)],
  ["eurostreaming", async (imdbid, season, episode) => await scrapeEurostreaming(imdbid, season, episode)]
]);
const info = /* @__PURE__ */ new Map([
  ["frembed", { name: "FRembed", lang_emoji: "\u{1F1EB}\u{1F1F7}" }],
  ["meinecloud", { name: "Meinecloud", lang_emoji: "\u{1F1E9}\u{1F1EA}" }],
  ["kinokiste", { name: "Kinokiste", lang_emoji: "\u{1F1E9}\u{1F1EA}" }],
  ["cinehdplus", { name: "CineHDplus", lang_emoji: "\u{1F1EA}\u{1F1F8}\u{1F1F2}\u{1F1FD}" }],
  ["verhdlink", { name: "VerHDlink", lang_emoji: "\u{1F1EA}\u{1F1F8}\u{1F1F2}\u{1F1FD}" }],
  ["eurostreaming", { name: "EuroStreaming", lang_emoji: "\u{1F1EE}\u{1F1F9}" }],
  ["guardahd", { name: "GuardaHD", lang_emoji: "\u{1F1EE}\u{1F1F9}" }]
]);
async function scrapeCustomProviders(list, imdb, season, episode) {
  const output = {
    streams: []
  };
  let sourcelist = list.split(",");
  if (episode == 0 || episode == "0") {
    for (let source of sourcelist) {
      if (source == "en" && scrape_english == "true") {
        ...
        for (const source2 of sources) {
          const stream = await getMedia(media, source2);
              ...
              if (streamItem.type === "file") {
                for (const qualityKey in streamItem.qualities) {
                  ...
                  output.streams.push({
                    ...
                    type: "url",
                    url: quality.url,
                    title: `${source2} - ${qualityKey}p (${embed})`
                  });
                }
...
async function buildHTMLselectors() {
  let selector = "";
  if (scrape_english == "true") {
    selector = `${selector}       
            <input type="checkbox" id="en" name="en" value="en">
            <label for="en">\u{1F1EC}\u{1F1E7} Built-in English Providers</label><br>`;
  }
...
const _7kRd6B = defineEventHandler(async (event) => {
...
const _T1aWet = eventHandler((event) => {
  const { req, res } = event;
  ...
  res.end = (chunk, encoding, cb) => {
    ...
    const durationInSeconds = (Date.now() - startTime) / 1e3;
...
const _lazy_3XFLQU = () => import('./routes/catalog.json.mjs');
const _lazy_Fj1d1q = () => import('./routes/index.mjs');
const _lazy_r0p5vu = () => import('./routes/manifest.json.mjs');
const _lazy_j8hg4y = () => import('./routes/stream/movie/_imdb_.mjs');
const _lazy_UINK9W = () => import('./routes/stream/series/_imdb_.mjs');

const handlers = [
  { route: '', handler: _f4b49z, lazy: false, middleware: true, method: undefined },
  { route: '', handler: _UGPm8R, lazy: false, middleware: true, method: undefined },
  { route: '', handler: _7kRd6B, lazy: false, middleware: true, method: undefined },
  { route: '', handler: _T1aWet, lazy: false, middleware: true, method: undefined },
  { route: '/catalog.json', handler: _lazy_3XFLQU, lazy: true, middleware: false, method: undefined },
  { route: '/', handler: _lazy_Fj1d1q, lazy: true, middleware: false, method: undefined },
  { route: '/manifest.json', handler: _lazy_r0p5vu, lazy: true, middleware: false, method: undefined },
  { route: '/stream/movie/:imdb', handler: _lazy_j8hg4y, lazy: true, middleware: false, method: undefined },
  { route: '/stream/series/:imdb', handler: _lazy_UINK9W, lazy: true, middleware: false, method: undefined }
];
...
export { getMovieMediaDetails as a, buildHTMLselectors as b, convertImdbIdToTmdbId as c, getMedia as d, eventHandler as e, scrapeGuardahd as f, getRouterParam as g, scrapeFrembed as h, scrapeMeinecloud as i, scrapeVerdahd as j, scrapeEurostreaming as k, scrapeKinokiste as l, scrapeCinehdplus as m, getShowMediaDetails as n, nodeServer as o, setHeader as s };
